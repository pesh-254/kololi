...
async function bundesligaStandingsCommand(sock, chatId, message) {
    try {
        ...
        const apiUrl = "https://api.dreaded.site/api/standings/BL1";
        const response = await axios.get(apiUrl);

        if (!response.data || !response.data.data) {
            await sock.sendMessage(chatId, { 
                text: '❌ Unable to fetch Bundesliga standings. Please try again later.' 
            }, { quoted: fake });
            return;
        }
        ...
        let standingsList = `⚽ *BUNDESLIGA TABLE STANDINGS* ⚽\n\n`;
        ...
        // Send error message
        await sock.sendMessage(chatId, { 
            text: '❌ Something went wrong. Unable to fetch Bundesliga standings.' 
        }, { quoted: fake });
