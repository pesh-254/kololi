...
async function serieAStandingsCommand(sock, chatId, message) {
    try {
        ...
        const apiUrl = "https://api.dreaded.site/api/standings/SA";
        const response = await axios.get(apiUrl);

        if (!response.data || !response.data.data) {
            await sock.sendMessage(chatId, { 
                text: '❌ Unable to fetch Serie A standings. Please try again later.' 
            }, { quoted: fake });
            return;
        }
        ...
        let standingsList = `⚽ *SERIE-A TABLE STANDINGS* ⚽\n\n`;
        ...
        // Send error message
        await sock.sendMessage(chatId, { 
            text: '❌ Something went wrong. Unable to fetch Serie A standings.' 
        }, { quoted: fake });
