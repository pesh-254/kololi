...
async function toVoiceNoteCommand(sock, chatId, message) {
  ...
  try {
    ...
    if (!msg) {
      await sock.sendMessage(chatId, { 
        text: "Reply to video or audio to convert."
      }, { quoted: fkontak });
      return;
    }
    ...
    if (!isVideo && !isAudio && !isDocument) {
      await sock.sendMessage(chatId, { 
        text: "Video or audio only."
      }, { quoted: fkontak });
      return;
    }

    await sock.sendMessage(chatId, { text: "Converting to voice..." }, { quoted: fkontak });
    ...
    outputPath = path.join(tempDir, `output_${timestamp}_${random}.opus`);
...
module.exports = toVoiceNoteCommand;
