...
async function getLoginMethod() {
    ...
    if (choice === '1') {
        ...
        if (!sessionId.includes("DAVE-AI:~")) { 
            log("Invalid Session ID format! Must contain 'DAVE-X:~'.", 'red'); 
            process.exit(1); 
        }
...
┗━━━━━━━━━━━━━━━━━━━━━`, 
            ...
            console.log('[DAVE-X] Startup welcome message disabled.');
        }
...
XeonBotInc.ev.on('messages.update', async (messageUpdates) => {
    try {
        ...
        console.error('[ANTIEDIT ERROR]', error.message, 'Line:', error.stack?.split('\n')[1]);
    }
});
